#FINAL: SWITZERLAND DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - SWITZERLAND\n")
...
# 1.1 CBR Data (Original for Switzerland)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
switzerland_cbr <- cbr_all %>%
  filter(country == "Switzerland") %>%
  arrange(year)

cat("✓ CBR Switzerland (original):", nrow(switzerland_cbr), "rows\n")
cat("  Range:", min(switzerland_cbr$year), "-", max(switzerland_cbr$year), "\n")
print(switzerland_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - SWITZERLAND\n")
cat("========================================\n\n")

vaccine_switzerland <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  # Find Switzerland row
  switzerland_row <- NULL
  for(i in 1:nrow(df_vaccine_raw)) {
    row_vals <- as.character(df_vaccine_raw[i, ])
    if(any(grepl("Switzerland", row_vals, ignore.case = TRUE))) {
      switzerland_row <- i
      break
    }
  }
  
  if(!is.null(switzerland_row)) {
    coverage_raw <- df_vaccine_raw[switzerland_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Switzerland vaccine coverage (original):\n")
    ...
    cat("\n✓ Switzerland vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_switzerland_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_switzerland_interpolated.csv\n")
  } else {
    cat("⚠️ Switzerland not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - SWITZERLAND\n")
cat("========================================\n\n")

# Filter Switzerland
switzerland_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Switzerland" & year >= 2000 & year <= 2025)

cat("✓ Switzerland deaths:", nrow(switzerland_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
switzerland_deaths <- switzerland_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(switzerland_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- switzerland_deaths %>%
...
excess_winter <- switzerland_deaths %>%
  filter(is_winter & !is.na(week)) %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - SWITZERLAND\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  switzerland_sentinel <- df_sentinel %>%
  ...
  cat("✓ Switzerland sentinel:", nrow(switzerland_sentinel), "rows\n")
  
  # Define Winter weeks
  switzerland_sentinel <- switzerland_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(switzerland_sentinel), "\n")
  
  if(nrow(switzerland_sentinel) > 0) {
    sentinel_wide <- switzerland_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - SWITZERLAND\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - SWITZERLAND\n")
cat("========================================\n\n")

# Temperature weekly
switzerland_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(switzerland_temp_weekly, by = c("year", "week")) %>%
  left_join(switzerland_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - SWITZERLAND\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - SWITZERLAND\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - SWITZERLAND\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - SWITZERLAND\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - SWITZERLAND\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - SWITZERLAND
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - SWITZERLAND\n")
cat("========================================\n\n")

cat("ANALYSIS: Switzerland\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - SWITZERLAND\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - SWITZERLAND!\n")
cat("========================================\n")
