#FINAL: CYPRUS DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - CYPRUS\n")
...
# 1.1 CBR Data (Original for Cyprus)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
cyprus_cbr <- cbr_all %>%
  filter(country == "Cyprus") %>%
  arrange(year)

cat("✓ CBR Cyprus (original):", nrow(cyprus_cbr), "rows\n")
cat("  Range:", min(cyprus_cbr$year), "-", max(cyprus_cbr$year), "\n")
print(cyprus_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - CYPRUS
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - CYPRUS\n")
cat("========================================\n\n")
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Cyprus", row_vals, ignore.case = TRUE))) {
      cyprus_row <- i
      break
    }
  }
  
  if(!is.null(cyprus_row)) {
    coverage_raw <- df_vaccine_raw[cyprus_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Cyprus vaccine coverage (original):\n")
    ...
    cat("\n✓ Cyprus vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_cyprus_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_cyprus_interpolated.csv\n")
  } else {
    cat("⚠️ Cyprus not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - CYPRUS
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - CYPRUS\n")
cat("========================================\n\n")

# Filter Cyprus
cyprus_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Cyprus" & year >= 2000 & year <= 2025)

cat("✓ Cyprus deaths:", nrow(cyprus_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
cyprus_deaths <- cyprus_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(cyprus_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- cyprus_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - CYPRUS
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - CYPRUS\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Cyprus sentinel:", nrow(cyprus_sentinel), "rows\n")
  
  # Define Winter weeks
  cyprus_sentinel <- cyprus_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(cyprus_sentinel), "\n")
  
  if(nrow(cyprus_sentinel) > 0) {
    sentinel_wide <- cyprus_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - CYPRUS
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - CYPRUS\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - CYPRUS
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - CYPRUS\n")
cat("========================================\n\n")

# Temperature weekly
cyprus_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(cyprus_temp_weekly, by = c("year", "week")) %>%
  left_join(cyprus_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - CYPRUS
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - CYPRUS\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - CYPRUS\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - CYPRUS\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - CYPRUS
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - CYPRUS\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - CYPRUS\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - CYPRUS
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - CYPRUS\n")
cat("========================================\n\n")
