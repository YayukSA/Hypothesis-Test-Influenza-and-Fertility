#FINAL: GREECE DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - GREECE\n")
...
# 1.1 CBR Data (Original for Greece)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
greece_cbr <- cbr_all %>%
  filter(country == "Greece") %>%
  arrange(year)

cat("✓ CBR Greece (original):", nrow(greece_cbr), "rows\n")
cat("  Range:", min(greece_cbr$year), "-", max(greece_cbr$year), "\n")
print(greece_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - GREECE
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - GREECE\n")
cat("========================================\n\n")
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Greece", row_vals, ignore.case = TRUE))) {
      greece_row <- i
      break
    }
  }
  
  if(!is.null(greece_row)) {
    coverage_raw <- df_vaccine_raw[greece_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Greece vaccine coverage (original):\n")
    ...
    cat("\n✓ Greece vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_greece_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_greece_interpolated.csv\n")
  } else {
    cat("⚠️ Greece not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - GREECE
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - GREECE\n")
cat("========================================\n\n")

# Filter Greece
greece_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Greece" & year >= 2000 & year <= 2025)

cat("✓ Greece deaths:", nrow(greece_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
greece_deaths <- greece_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(greece_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- greece_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - GREECE
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - GREECE\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Greece sentinel:", nrow(greece_sentinel), "rows\n")
  
  # Define Winter weeks
  greece_sentinel <- greece_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(greece_sentinel), "\n")
  
  if(nrow(greece_sentinel) > 0) {
    sentinel_wide <- greece_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - GREECE
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - GREECE\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - GREECE
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - GREECE\n")
cat("========================================\n\n")

# Temperature weekly
greece_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(greece_temp_weekly, by = c("year", "week")) %>%
  left_join(greece_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - GREECE
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - GREECE\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - GREECE\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - GREECE\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - GREECE
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - GREECE\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - GREECE\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - GREECE
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - GREECE\n")
cat("========================================\n\n")
