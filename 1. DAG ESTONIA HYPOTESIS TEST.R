#FINAL: ESTONIA DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - ESTONIA\n")
...
# 1.1 CBR Data (Original for Estonia)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
estonia_cbr <- cbr_all %>%
  filter(country == "Estonia") %>%
  arrange(year)

cat("✓ CBR Estonia (original):", nrow(estonia_cbr), "rows\n")
cat("  Range:", min(estonia_cbr$year), "-", max(estonia_cbr$year), "\n")
print(estonia_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - ESTONIA
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - ESTONIA\n")
cat("========================================\n\n")

vaccine_estonia <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Estonia", row_vals, ignore.case = TRUE))) {
      estonia_row <- i
      break
    }
  }
  
  if(!is.null(estonia_row)) {
    coverage_raw <- df_vaccine_raw[estonia_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Estonia vaccine coverage (original):\n")
    ...
    cat("\n✓ Estonia vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_estonia_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_estonia_interpolated.csv\n")
  } else {
    cat("⚠️ Estonia not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - ESTONIA
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - ESTONIA\n")
cat("========================================\n\n")

# Filter Estonia
estonia_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Estonia" & year >= 2000 & year <= 2025)

cat("✓ Estonia deaths:", nrow(estonia_deaths), "rows\n")
...
cat("  Winter weeks:", sum(estonia_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- estonia_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - ESTONIA
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - ESTONIA\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Estonia sentinel:", nrow(estonia_sentinel), "rows\n")
  
  # Define Winter weeks
  estonia_sentinel <- estonia_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(estonia_sentinel), "\n")
  
  if(nrow(estonia_sentinel) > 0) {
    sentinel_wide <- estonia_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - ESTONIA
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - ESTONIA\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - ESTONIA
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - ESTONIA\n")
cat("========================================\n\n")

# Temperature weekly
estonia_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(estonia_temp_weekly, by = c("year", "week")) %>%
  left_join(estonia_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - ESTONIA
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - ESTONIA\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - ESTONIA\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - ESTONIA\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - ESTONIA
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - ESTONIA\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - ESTONIA\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - ESTONIA
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - ESTONIA\n")
cat("========================================\n\n")

cat("ANALYSIS: Estonia\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - ESTONIA\n")
  cat("========================================\n\n")
