#FINAL: BELGIUM DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - BELGIUM\n")
...
# 1.1 CBR Data (Original for Belgium)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
belgium_cbr <- cbr_all %>%
  filter(country == "Belgium") %>%
  arrange(year)

cat("✓ CBR Belgium (original):", nrow(belgium_cbr), "rows\n")
cat("  Range:", min(belgium_cbr$year), "-", max(belgium_cbr$year), "\n")
print(belgium_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - BELGIUM
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - BELGIUM\n")
cat("========================================\n\n")

vaccine_belgium <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Belgium", row_vals, ignore.case = TRUE))) {
      belgium_row <- i
      break
    }
  }
  
  if(!is.null(belgium_row)) {
    coverage_raw <- df_vaccine_raw[belgium_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Belgium vaccine coverage (original):\n")
    ...
    cat("\n✓ Belgium vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_belgium_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_belgium_interpolated.csv\n")
  } else {
    cat("⚠️ Belgium not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - BELGIUM
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - BELGIUM\n")
cat("========================================\n\n")

# Filter Belgium
belgium_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Belgium" & year >= 2000 & year <= 2025)

cat("✓ Belgium deaths:", nrow(belgium_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
belgium_deaths <- belgium_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(belgium_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- belgium_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - BELGIUM
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - BELGIUM\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Belgium sentinel:", nrow(belgium_sentinel), "rows\n")
  
  # Define Winter weeks
  belgium_sentinel <- belgium_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(belgium_sentinel), "\n")
  
  if(nrow(belgium_sentinel) > 0) {
    sentinel_wide <- belgium_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - BELGIUM
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - BELGIUM\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - BELGIUM
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - BELGIUM\n")
cat("========================================\n\n")

# Temperature weekly
belgium_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(belgium_temp_weekly, by = c("year", "week")) %>%
  left_join(belgium_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - BELGIUM
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - BELGIUM\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - BELGIUM\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - BELGIUM\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - BELGIUM
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - BELGIUM\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - BELGIUM\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - BELGIUM
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - BELGIUM\n")
cat("========================================\n\n")

cat("ANALYSIS: Belgium\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - BELGIUM\n")
  cat("========================================\n\n")
