#FINAL: FRANCE DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - FRANCE\n")
...
# 1.1 CBR Data (Original for France)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
france_cbr <- cbr_all %>%
  filter(country == "France") %>%
  arrange(year)

cat("✓ CBR France (original):", nrow(france_cbr), "rows\n")
cat("  Range:", min(france_cbr$year), "-", max(france_cbr$year), "\n")
print(france_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - FRANCE
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - FRANCE\n")
cat("========================================\n\n")
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("France", row_vals, ignore.case = TRUE))) {
      france_row <- i
      break
    }
  }
  
  if(!is.null(france_row)) {
    coverage_raw <- df_vaccine_raw[france_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ France vaccine coverage (original):\n")
    ...
    cat("\n✓ France vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_france_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_france_interpolated.csv\n")
  } else {
    cat("⚠️ France not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - FRANCE
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - FRANCE\n")
cat("========================================\n\n")

# Filter France
france_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "France" & year >= 2000 & year <= 2025)

cat("✓ France deaths:", nrow(france_deaths), "rows\n")
...
cat("  Winter weeks:", sum(france_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- france_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - FRANCE
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - FRANCE\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ France sentinel:", nrow(france_sentinel), "rows\n")
  
  # Define Winter weeks
  france_sentinel <- france_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(france_sentinel), "\n")
  
  if(nrow(france_sentinel) > 0) {
    sentinel_wide <- france_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - FRANCE
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - FRANCE\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - FRANCE
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - FRANCE\n")
cat("========================================\n\n")

# Temperature weekly
france_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(france_temp_weekly, by = c("year", "week")) %>%
  left_join(france_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - FRANCE
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - FRANCE\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - FRANCE\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - FRANCE\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - FRANCE
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - FRANCE\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - FRANCE\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - FRANCE
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - FRANCE\n")
cat("========================================\n\n")
