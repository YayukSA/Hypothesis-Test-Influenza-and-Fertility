#FINAL: LUXEMBOURG DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - LUXEMBOURG\n")
...
# 1.1 CBR Data (Original for Luxembourg)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
luxembourg_cbr <- cbr_all %>%
  filter(country == "Luxembourg") %>%
  arrange(year)

cat("✓ CBR Luxembourg (original):", nrow(luxembourg_cbr), "rows\n")
cat("  Range:", min(luxembourg_cbr$year), "-", max(luxembourg_cbr$year), "\n")
print(luxembourg_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - LUXEMBOURG\n")
cat("========================================\n\n")

vaccine_luxembourg <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Luxembourg", row_vals, ignore.case = TRUE))) {
      luxembourg_row <- i
      break
    }
  }
  
  if(!is.null(luxembourg_row)) {
    coverage_raw <- df_vaccine_raw[luxembourg_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Luxembourg vaccine coverage (original):\n")
    ...
    cat("\n✓ Luxembourg vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_luxembourg_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_luxembourg_interpolated.csv\n")
  } else {
    cat("⚠️ Luxembourg not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - LUXEMBOURG\n")
cat("========================================\n\n")

# Filter Luxembourg
luxembourg_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Luxembourg" & year >= 2000 & year <= 2025)

cat("✓ Luxembourg deaths:", nrow(luxembourg_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
luxembourg_deaths <- luxembourg_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(luxembourg_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- luxembourg_deaths %>%
...
excess_winter <- luxembourg_deaths %>%
  filter(is_winter & !is.na(week)) %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - LUXEMBOURG\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  luxembourg_sentinel <- df_sentinel %>%
    filter(countryname == "Luxembourg" & 
             pathogen == "Influenza" & 
  ...
  cat("✓ Luxembourg sentinel:", nrow(luxembourg_sentinel), "rows\n")
  
  # Define Winter weeks
  luxembourg_sentinel <- luxembourg_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(luxembourg_sentinel), "\n")
  
  if(nrow(luxembourg_sentinel) > 0) {
    sentinel_wide <- luxembourg_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - LUXEMBOURG\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - LUXEMBOURG\n")
cat("========================================\n\n")

# Temperature weekly
luxembourg_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(luxembourg_temp_weekly, by = c("year", "week")) %>%
  left_join(luxembourg_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - LUXEMBOURG\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - LUXEMBOURG\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - LUXEMBOURG\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - LUXEMBOURG\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - LUXEMBOURG\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - LUXEMBOURG
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - LUXEMBOURG\n")
cat("========================================\n\n")

cat("ANALYSIS: Luxembourg\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - LUXEMBOURG\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - LUXEMBOURG!\n")
cat("========================================\n")
