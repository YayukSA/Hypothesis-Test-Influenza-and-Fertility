#FINAL: GERMANY DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - GERMANY (FINAL)\n")
...
# 1.1 CBR Data (Interpolated for Germany)
germany_cbr <- read.csv(file.path(folder_path, "germany_cbr_interpolated.csv"))
cat("✓ CBR Germany (interpolated):", nrow(germany_cbr), "rows\n")
cat("  Range:", min(germany_cbr$year), "-", max(germany_cbr$year), "\n")
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - GERMANY
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - GERMANY\n")
cat("========================================\n\n")

vaccine_germany <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Germany", row_vals, ignore.case = TRUE))) {
      germany_row <- i
      break
    }
  }
  
  if(!is.null(germany_row)) {
    coverage_raw <- df_vaccine_raw[germany_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Germany vaccine coverage (original):\n")
    ...
    cat("\n✓ Germany vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_germany_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_germany_interpolated.csv\n")
  } else {
    cat("⚠️ Germany not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - GERMANY
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - GERMANY\n")
cat("========================================\n\n")

# Filter Germany
germany_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Germany" & year >= 2000 & year <= 2025)

cat("✓ Germany deaths:", nrow(germany_deaths), "rows\n")
...
cat("  Winter weeks:", sum(germany_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- germany_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - GERMANY
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - GERMANY\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Germany sentinel:", nrow(germany_sentinel), "rows\n")
  
  # Define Winter weeks
  germany_sentinel <- germany_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(germany_sentinel), "\n")
  
  if(nrow(germany_sentinel) > 0) {
    sentinel_wide <- germany_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - GERMANY
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - GERMANY\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - GERMANY
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - GERMANY\n")
cat("========================================\n\n")

# Temperature weekly
germany_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(germany_temp_weekly, by = c("year", "week")) %>%
  left_join(germany_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - GERMANY
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - GERMANY\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - GERMANY\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - GERMANY\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - GERMANY
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - GERMANY\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - GERMANY\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - GERMANY
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - GERMANY\n")
cat("========================================\n\n")

cat("ANALYSIS: Germany\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - GERMANY\n")
  cat("========================================\n\n")
