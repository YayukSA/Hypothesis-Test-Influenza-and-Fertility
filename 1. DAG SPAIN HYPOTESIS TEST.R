#FINAL: SPAIN DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - SPAIN\n")
...
# 1.1 CBR Data (Original for Spain)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
spain_cbr <- cbr_all %>%
  filter(country == "Spain") %>%
  arrange(year)

cat("✓ CBR Spain (original):", nrow(spain_cbr), "rows\n")
cat("  Range:", min(spain_cbr$year), "-", max(spain_cbr$year), "\n")
print(spain_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - SPAIN
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - SPAIN\n")
cat("========================================\n\n")
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Spain", row_vals, ignore.case = TRUE))) {
      spain_row <- i
      break
    }
  }
  
  if(!is.null(spain_row)) {
    coverage_raw <- df_vaccine_raw[spain_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Spain vaccine coverage (original):\n")
    ...
    cat("\n✓ Spain vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_spain_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_spain_interpolated.csv\n")
  } else {
    cat("⚠️ Spain not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - SPAIN
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - SPAIN\n")
cat("========================================\n\n")

# Filter Spain
spain_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Spain" & year >= 2000 & year <= 2025)

cat("✓ Spain deaths:", nrow(spain_deaths), "rows\n")
...
cat("  Winter weeks:", sum(spain_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- spain_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - SPAIN
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - SPAIN\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Spain sentinel:", nrow(spain_sentinel), "rows\n")
  
  # Define Winter weeks
  spain_sentinel <- spain_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(spain_sentinel), "\n")
  
  if(nrow(spain_sentinel) > 0) {
    sentinel_wide <- spain_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - SPAIN
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - SPAIN\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - SPAIN
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - SPAIN\n")
cat("========================================\n\n")

# Temperature weekly
spain_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(spain_temp_weekly, by = c("year", "week")) %>%
  left_join(spain_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - SPAIN
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - SPAIN\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - SPAIN\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - SPAIN\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - SPAIN
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - SPAIN\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - SPAIN\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - SPAIN
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - SPAIN\n")
cat("========================================\n\n")
