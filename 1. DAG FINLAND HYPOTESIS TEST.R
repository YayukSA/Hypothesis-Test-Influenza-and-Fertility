#FINAL: FINLAND DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - FINLAND\n")
...
# 1.1 CBR Data (Original for Finland)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
finland_cbr <- cbr_all %>%
  filter(country == "Finland") %>%
  arrange(year)

cat("✓ CBR Finland (original):", nrow(finland_cbr), "rows\n")
cat("  Range:", min(finland_cbr$year), "-", max(finland_cbr$year), "\n")
print(finland_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - FINLAND
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - FINLAND\n")
cat("========================================\n\n")

vaccine_finland <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Finland", row_vals, ignore.case = TRUE))) {
      finland_row <- i
      break
    }
  }
  
  if(!is.null(finland_row)) {
    coverage_raw <- df_vaccine_raw[finland_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Finland vaccine coverage (original):\n")
    ...
    cat("\n✓ Finland vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_finland_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_finland_interpolated.csv\n")
  } else {
    cat("⚠️ Finland not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - FINLAND
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - FINLAND\n")
cat("========================================\n\n")

# Filter Finland
finland_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Finland" & year >= 2000 & year <= 2025)

cat("✓ Finland deaths:", nrow(finland_deaths), "rows\n")
...
cat("  Winter weeks:", sum(finland_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- finland_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - FINLAND
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - FINLAND\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Finland sentinel:", nrow(finland_sentinel), "rows\n")
  
  # Define Winter weeks
  finland_sentinel <- finland_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(finland_sentinel), "\n")
  
  if(nrow(finland_sentinel) > 0) {
    sentinel_wide <- finland_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - FINLAND
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - FINLAND\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - FINLAND
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - FINLAND\n")
cat("========================================\n\n")

# Temperature weekly
finland_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(finland_temp_weekly, by = c("year", "week")) %>%
  left_join(finland_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - FINLAND
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - FINLAND\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - FINLAND\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - FINLAND\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - FINLAND
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - FINLAND\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - FINLAND\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - FINLAND
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - FINLAND\n")
cat("========================================\n\n")

cat("ANALYSIS: Finland\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - FINLAND\n")
  cat("========================================\n\n")
