#FINAL: SLOVAKIA DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - SLOVAKIA\n")
...
# 1.1 CBR Data (Original for Slovakia)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
slovakia_cbr <- cbr_all %>%
  filter(country == "Slovakia") %>%
  arrange(year)

cat("✓ CBR Slovakia (original):", nrow(slovakia_cbr), "rows\n")
cat("  Range:", min(slovakia_cbr$year), "-", max(slovakia_cbr$year), "\n")
print(slovakia_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - SLOVAKIA\n")
cat("========================================\n\n")

vaccine_slovakia <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Slovakia", row_vals, ignore.case = TRUE))) {
      slovakia_row <- i
      break
    }
  }
  
  if(!is.null(slovakia_row)) {
    coverage_raw <- df_vaccine_raw[slovakia_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Slovakia vaccine coverage (original):\n")
    ...
    cat("\n✓ Slovakia vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_slovakia_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_slovakia_interpolated.csv\n")
  } else {
    cat("⚠️ Slovakia not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - SLOVAKIA\n")
cat("========================================\n\n")

# Filter Slovakia
slovakia_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Slovakia" & year >= 2000 & year <= 2025)

cat("✓ Slovakia deaths:", nrow(slovakia_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
slovakia_deaths <- slovakia_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(slovakia_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- slovakia_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - SLOVAKIA\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Slovakia sentinel:", nrow(slovakia_sentinel), "rows\n")
  
  # Define Winter weeks
  slovakia_sentinel <- slovakia_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(slovakia_sentinel), "\n")
  
  if(nrow(slovakia_sentinel) > 0) {
    sentinel_wide <- slovakia_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - SLOVAKIA\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - SLOVAKIA\n")
cat("========================================\n\n")

# Temperature weekly
slovakia_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(slovakia_temp_weekly, by = c("year", "week")) %>%
  left_join(slovakia_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - SLOVAKIA\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - SLOVAKIA\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - SLOVAKIA\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - SLOVAKIA\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - SLOVAKIA\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - SLOVAKIA
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - SLOVAKIA\n")
cat("========================================\n\n")

cat("ANALYSIS: Slovakia\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - SLOVAKIA\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - SLOVAKIA!\n")
cat("========================================\n")
