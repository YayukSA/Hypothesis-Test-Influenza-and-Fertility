#FINAL: SLOVENIA DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - SLOVENIA\n")
...
# 1.1 CBR Data (Original for Slovenia)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
slovenia_cbr <- cbr_all %>%
  filter(country == "Slovenia") %>%
  arrange(year)

cat("✓ CBR Slovenia (original):", nrow(slovenia_cbr), "rows\n")
cat("  Range:", min(slovenia_cbr$year), "-", max(slovenia_cbr$year), "\n")
print(slovenia_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - SLOVENIA\n")
cat("========================================\n\n")

vaccine_slovenia <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Slovenia", row_vals, ignore.case = TRUE))) {
      slovenia_row <- i
      break
    }
  }
  
  if(!is.null(slovenia_row)) {
    coverage_raw <- df_vaccine_raw[slovenia_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Slovenia vaccine coverage (original):\n")
    ...
    cat("\n✓ Slovenia vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_slovenia_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_slovenia_interpolated.csv\n")
  } else {
    cat("⚠️ Slovenia not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - SLOVENIA\n")
cat("========================================\n\n")

# Filter Slovenia
slovenia_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Slovenia" & year >= 2000 & year <= 2025)

cat("✓ Slovenia deaths:", nrow(slovenia_deaths), "rows\n")
...
cat("  Winter weeks:", sum(slovenia_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- slovenia_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - SLOVENIA\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Slovenia sentinel:", nrow(slovenia_sentinel), "rows\n")
  
  # Define Winter weeks
  slovenia_sentinel <- slovenia_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(slovenia_sentinel), "\n")
  
  if(nrow(slovenia_sentinel) > 0) {
    sentinel_wide <- slovenia_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - SLOVENIA\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - SLOVENIA\n")
cat("========================================\n\n")

# Temperature weekly
slovenia_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(slovenia_temp_weekly, by = c("year", "week")) %>%
  left_join(slovenia_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - SLOVENIA\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - SLOVENIA\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - SLOVENIA\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - SLOVENIA\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - SLOVENIA\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - SLOVENIA
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - SLOVENIA\n")
cat("========================================\n\n")

cat("ANALYSIS: Slovenia\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - SLOVENIA\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - SLOVENIA!\n")
cat("========================================\n")
