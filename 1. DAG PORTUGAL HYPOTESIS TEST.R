#FINAL: PORTUGAL DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - PORTUGAL\n")
...
# 1.1 CBR Data (Original for Portugal)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
portugal_cbr <- cbr_all %>%
  filter(country == "Portugal") %>%
  arrange(year)

cat("✓ CBR Portugal (original):", nrow(portugal_cbr), "rows\n")
cat("  Range:", min(portugal_cbr$year), "-", max(portugal_cbr$year), "\n")
print(portugal_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - PORTUGAL\n")
cat("========================================\n\n")

vaccine_portugal <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Portugal", row_vals, ignore.case = TRUE))) {
      portugal_row <- i
      break
    }
  }
  
  if(!is.null(portugal_row)) {
    coverage_raw <- df_vaccine_raw[portugal_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Portugal vaccine coverage (original):\n")
    ...
    cat("\n✓ Portugal vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_portugal_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_portugal_interpolated.csv\n")
  } else {
    cat("⚠️ Portugal not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - PORTUGAL\n")
cat("========================================\n\n")

# Filter Portugal
portugal_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Portugal" & year >= 2000 & year <= 2025)

cat("✓ Portugal deaths:", nrow(portugal_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
portugal_deaths <- portugal_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(portugal_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- portugal_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - PORTUGAL\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Portugal sentinel:", nrow(portugal_sentinel), "rows\n")
  
  # Define Winter weeks
  portugal_sentinel <- portugal_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(portugal_sentinel), "\n")
  
  if(nrow(portugal_sentinel) > 0) {
    sentinel_wide <- portugal_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - PORTUGAL\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - PORTUGAL\n")
cat("========================================\n\n")

# Temperature weekly
portugal_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(portugal_temp_weekly, by = c("year", "week")) %>%
  left_join(portugal_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - PORTUGAL\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - PORTUGAL\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - PORTUGAL\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - PORTUGAL\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - PORTUGAL\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - PORTUGAL
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - PORTUGAL\n")
cat("========================================\n\n")

cat("ANALYSIS: Portugal\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - PORTUGAL\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - PORTUGAL!\n")
cat("========================================\n")
