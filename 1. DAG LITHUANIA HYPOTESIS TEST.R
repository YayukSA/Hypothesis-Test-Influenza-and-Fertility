#FINAL: LITHUANIA DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - LITHUANIA\n")
...
# 1.1 CBR Data (Original for Lithuania)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
lithuania_cbr <- cbr_all %>%
  filter(country == "Lithuania") %>%
  arrange(year)

cat("✓ CBR Lithuania (original):", nrow(lithuania_cbr), "rows\n")
cat("  Range:", min(lithuania_cbr$year), "-", max(lithuania_cbr$year), "\n")
print(lithuania_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - LITHUANIA\n")
cat("========================================\n\n")

vaccine_lithuania <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Lithuania", row_vals, ignore.case = TRUE))) {
      lithuania_row <- i
      break
    }
  }
  
  if(!is.null(lithuania_row)) {
    coverage_raw <- df_vaccine_raw[lithuania_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Lithuania vaccine coverage (original):\n")
    ...
    cat("\n✓ Lithuania vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_lithuania_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_lithuania_interpolated.csv\n")
  } else {
    cat("⚠️ Lithuania not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - LITHUANIA\n")
cat("========================================\n\n")

# Filter Lithuania
lithuania_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Lithuania" & year >= 2000 & year <= 2025)

cat("✓ Lithuania deaths:", nrow(lithuania_deaths), "rows\n")
...
cat("  Winter weeks:", sum(lithuania_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- lithuania_deaths %>%
...
excess_winter <- lithuania_deaths %>%
  filter(is_winter & !is.na(week)) %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - LITHUANIA\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  lithuania_sentinel <- df_sentinel %>%
  ...
  cat("✓ Lithuania sentinel:", nrow(lithuania_sentinel), "rows\n")
  
  # Define Winter weeks
  lithuania_sentinel <- lithuania_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(lithuania_sentinel), "\n")
  
  if(nrow(lithuania_sentinel) > 0) {
    sentinel_wide <- lithuania_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - LITHUANIA\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - LITHUANIA\n")
cat("========================================\n\n")

# Temperature weekly
lithuania_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(lithuania_temp_weekly, by = c("year", "week")) %>%
  left_join(lithuania_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - LITHUANIA\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - LITHUANIA\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - LITHUANIA\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - LITHUANIA\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - LITHUANIA\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - LITHUANIA
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - LITHUANIA\n")
cat("========================================\n\n")

cat("ANALYSIS: Lithuania\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - LITHUANIA\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - LITHUANIA!\n")
cat("========================================\n")
