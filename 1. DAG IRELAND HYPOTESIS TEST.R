#FINAL: IRELAND DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - IRELAND\n")
...
# 1.1 CBR Data (Original for Ireland)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
ireland_cbr <- cbr_all %>%
  filter(country == "Ireland") %>%
  arrange(year)

cat("✓ CBR Ireland (original):", nrow(ireland_cbr), "rows\n")
cat("  Range:", min(ireland_cbr$year), "-", max(ireland_cbr$year), "\n")
print(ireland_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - IRELAND
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - IRELAND\n")
cat("========================================\n\n")

vaccine_ireland <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Ireland", row_vals, ignore.case = TRUE))) {
      ireland_row <- i
      break
    }
  }
  
  if(!is.null(ireland_row)) {
    coverage_raw <- df_vaccine_raw[ireland_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Ireland vaccine coverage (original):\n")
    ...
    cat("\n✓ Ireland vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_ireland_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_ireland_interpolated.csv\n")
  } else {
    cat("⚠️ Ireland not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - IRELAND
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - IRELAND\n")
cat("========================================\n\n")

# Filter Ireland
ireland_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Ireland" & year >= 2000 & year <= 2025)

cat("✓ Ireland deaths:", nrow(ireland_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
ireland_deaths <- ireland_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(ireland_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- ireland_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - IRELAND
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - IRELAND\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Ireland sentinel:", nrow(ireland_sentinel), "rows\n")
  
  # Define Winter weeks
  ireland_sentinel <- ireland_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(ireland_sentinel), "\n")
  
  if(nrow(ireland_sentinel) > 0) {
    sentinel_wide <- ireland_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - IRELAND
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - IRELAND\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - IRELAND
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - IRELAND\n")
cat("========================================\n\n")

# Temperature weekly
ireland_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(ireland_temp_weekly, by = c("year", "week")) %>%
  left_join(ireland_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - IRELAND
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - IRELAND\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - IRELAND\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - IRELAND\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - IRELAND
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - IRELAND\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - IRELAND\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - IRELAND
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - IRELAND\n")
cat("========================================\n\n")

cat("ANALYSIS: Ireland\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - IRELAND\n")
  cat("========================================\n\n")
