#FINAL: HUNGARY DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - HUNGARY\n")
...
# 1.1 CBR Data (Original for Hungary)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
hungary_cbr <- cbr_all %>%
  filter(country == "Hungary") %>%
  arrange(year)

cat("✓ CBR Hungary (original):", nrow(hungary_cbr), "rows\n")
cat("  Range:", min(hungary_cbr$year), "-", max(hungary_cbr$year), "\n")
print(hungary_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - HUNGARY
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - HUNGARY\n")
cat("========================================\n\n")

vaccine_hungary <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Hungary", row_vals, ignore.case = TRUE))) {
      hungary_row <- i
      break
    }
  }
  
  if(!is.null(hungary_row)) {
    coverage_raw <- df_vaccine_raw[hungary_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Hungary vaccine coverage (original):\n")
    ...
    cat("\n✓ Hungary vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_hungary_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_hungary_interpolated.csv\n")
  } else {
    cat("⚠️ Hungary not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - HUNGARY
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - HUNGARY\n")
cat("========================================\n\n")

# Filter Hungary
hungary_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Hungary" & year >= 2000 & year <= 2025)

cat("✓ Hungary deaths:", nrow(hungary_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
hungary_deaths <- hungary_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(hungary_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- hungary_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - HUNGARY
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - HUNGARY\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Hungary sentinel:", nrow(hungary_sentinel), "rows\n")
  
  # Define Winter weeks
  hungary_sentinel <- hungary_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(hungary_sentinel), "\n")
  
  if(nrow(hungary_sentinel) > 0) {
    sentinel_wide <- hungary_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - HUNGARY
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - HUNGARY\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - HUNGARY
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - HUNGARY\n")
cat("========================================\n\n")

# Temperature weekly
hungary_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(hungary_temp_weekly, by = c("year", "week")) %>%
  left_join(hungary_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - HUNGARY
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - HUNGARY\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - HUNGARY\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - HUNGARY\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - HUNGARY
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - HUNGARY\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - HUNGARY\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - HUNGARY
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - HUNGARY\n")
cat("========================================\n\n")

cat("ANALYSIS: Hungary\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - HUNGARY\n")
  cat("========================================\n\n")
