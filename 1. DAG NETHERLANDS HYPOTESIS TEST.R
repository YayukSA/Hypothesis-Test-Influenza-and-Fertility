#FINAL: NETHERLANDS DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - NETHERLANDS\n")
...
# 1.1 CBR Data (Original for Netherlands)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
netherlands_cbr <- cbr_all %>%
  filter(country == "Netherlands") %>%
  arrange(year)

cat("✓ CBR Netherlands (original):", nrow(netherlands_cbr), "rows\n")
cat("  Range:", min(netherlands_cbr$year), "-", max(netherlands_cbr$year), "\n")
print(netherlands_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - NETHERLANDS\n")
cat("========================================\n\n")

vaccine_netherlands <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  # Find Netherlands row
  netherlands_row <- NULL
  for(i in 1:nrow(df_vaccine_raw)) {
    row_vals <- as.character(df_vaccine_raw[i, ])
    if(any(grepl("Netherlands", row_vals, ignore.case = TRUE))) {
      netherlands_row <- i
      break
    }
  }
  
  if(!is.null(netherlands_row)) {
    coverage_raw <- df_vaccine_raw[netherlands_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Netherlands vaccine coverage (original):\n")
    ...
    cat("\n✓ Netherlands vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_netherlands_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_netherlands_interpolated.csv\n")
  } else {
    cat("⚠️ Netherlands not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - NETHERLANDS\n")
cat("========================================\n\n")

# Filter Netherlands
netherlands_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Netherlands" & year >= 2000 & year <= 2025)

cat("✓ Netherlands deaths:", nrow(netherlands_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
netherlands_deaths <- netherlands_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(netherlands_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- netherlands_deaths %>%
...
excess_winter <- netherlands_deaths %>%
  filter(is_winter & !is.na(week)) %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - NETHERLANDS\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  netherlands_sentinel <- df_sentinel %>%
  ...
  cat("✓ Netherlands sentinel:", nrow(netherlands_sentinel), "rows\n")
  
  # Define Winter weeks
  netherlands_sentinel <- netherlands_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(netherlands_sentinel), "\n")
  
  if(nrow(netherlands_sentinel) > 0) {
    sentinel_wide <- netherlands_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - NETHERLANDS\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - NETHERLANDS\n")
cat("========================================\n\n")

# Temperature weekly
netherlands_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(netherlands_temp_weekly, by = c("year", "week")) %>%
  left_join(netherlands_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - NETHERLANDS\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - NETHERLANDS\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - NETHERLANDS\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - NETHERLANDS\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - NETHERLANDS\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - NETHERLANDS
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - NETHERLANDS\n")
cat("========================================\n\n")

cat("ANALYSIS: Netherlands\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - NETHERLANDS\n")
  cat("========================================\n\n")
...
cat("\n========================================\n")
cat("ANALYSIS COMPLETE - NETHERLANDS!\n")
cat("========================================\n")