#FINAL: NORWAY DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - NORWAY\n")
...
# 1.1 CBR Data (Original for Norway)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
norway_cbr <- cbr_all %>%
  filter(country == "Norway") %>%
  arrange(year)

cat("✓ CBR Norway (original):", nrow(norway_cbr), "rows\n")
cat("  Range:", min(norway_cbr$year), "-", max(norway_cbr$year), "\n")
print(norway_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - NORWAY
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - NORWAY\n")
cat("========================================\n\n")
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Norway", row_vals, ignore.case = TRUE))) {
      norway_row <- i
      break
    }
  }
  
  if(!is.null(norway_row)) {
    coverage_raw <- df_vaccine_raw[norway_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Norway vaccine coverage (original):\n")
    ...
    cat("\n✓ Norway vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_norway_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_norway_interpolated.csv\n")
  } else {
    cat("⚠️ Norway not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - NORWAY
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - NORWAY\n")
cat("========================================\n\n")

# Filter Norway
norway_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Norway" & year >= 2000 & year <= 2025)

cat("✓ Norway deaths:", nrow(norway_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
norway_deaths <- norway_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(norway_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- norway_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - NORWAY
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - NORWAY\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Norway sentinel:", nrow(norway_sentinel), "rows\n")
  
  # Define Winter weeks
  norway_sentinel <- norway_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(norway_sentinel), "\n")
  
  if(nrow(norway_sentinel) > 0) {
    sentinel_wide <- norway_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - NORWAY
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - NORWAY\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - NORWAY
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - NORWAY\n")
cat("========================================\n\n")

# Temperature weekly
norway_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(norway_temp_weekly, by = c("year", "week")) %>%
  left_join(norway_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - NORWAY
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - NORWAY\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - NORWAY\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - NORWAY\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - NORWAY
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - NORWAY\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - NORWAY\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - NORWAY
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - NORWAY\n")
cat("========================================\n\n")
