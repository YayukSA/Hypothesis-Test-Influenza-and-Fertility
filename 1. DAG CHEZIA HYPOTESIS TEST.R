#FINAL: CZECHIA DAG: EXCESS MORTALITY (WEEKLY WINTER, 2000-2025)
...
cat("========================================\n")
cat("ANALISIS DAG LENGKAP - CZECHIA\n")
...
# 1.1 CBR Data (Original for Czechia)
cbr_all <- read.csv(file.path(folder_path, "birth_with_cbr_europe_2000_fixed.csv"))
czechia_cbr <- cbr_all %>%
  filter(country == "Czechia") %>%
  arrange(year)

cat("✓ CBR Czechia (original):", nrow(czechia_cbr), "rows\n")
cat("  Range:", min(czechia_cbr$year), "-", max(czechia_cbr$year), "\n")
print(czechia_cbr)
...
# ============================================
# 2. PROCESS VACCINE DATA WITH INTERPOLATION - CZECHIA
# ============================================

cat("\n========================================\n")
cat("2. PROCESSING VACCINE DATA (WITH INTERPOLATION) - CZECHIA\n")
cat("========================================\n\n")

vaccine_czechia <- NULL
vaccine_interp <- NULL
...
if(!is.null(df_vaccine_raw)) {
  ...
  for(i in 1:nrow(df_vaccine_raw)) {
    ...
    if(any(grepl("Czechia", row_vals, ignore.case = TRUE))) {
      czechia_row <- i
      break
    }
  }
  
  if(!is.null(czechia_row)) {
    coverage_raw <- df_vaccine_raw[czechia_row, 2:ncol(df_vaccine_raw)]
    ...
    cat("✓ Czechia vaccine coverage (original):\n")
    ...
    cat("\n✓ Czechia vaccine coverage (interpolated):\n")
    ...
    write.csv(vaccine_interp, 
              file.path(folder_path, "vaccine_czechia_interpolated.csv"),
              row.names = FALSE)
    cat("\n✅ Interpolated vaccine data saved to: vaccine_czechia_interpolated.csv\n")
  } else {
    cat("⚠️ Czechia not found in vaccine data\n")
  }
}

# ============================================
# 3. PROCESS EXCESS MORTALITY (WEEKLY WINTER) - CZECHIA
# ============================================

cat("\n========================================\n")
cat("3. PROCESSING EXCESS MORTALITY (WEEKLY WINTER) - CZECHIA\n")
cat("========================================\n\n")

# Filter Czechia
czechia_deaths <- all_data_final %>%
  mutate(
    year = as.numeric(substr(Week, 1, 4)),
    week = as.numeric(gsub(".*-W", "", Week)),
    country = `GEO (Labels)`
  ) %>%
  filter(country == "Czechia" & year >= 2000 & year <= 2025)

cat("✓ Czechia deaths:", nrow(czechia_deaths), "rows\n")

# Define Winter (weeks 40-52 and 1-9)
czechia_deaths <- czechia_deaths %>%
  mutate(
    is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
  )

cat("  Winter weeks:", sum(czechia_deaths$is_winter, na.rm = TRUE), "\n")

# Calculate baseline mortality for Winter
baseline_winter <- czechia_deaths %>%
...
# ============================================
# 4. PROCESS SENTINEL (WEEKLY WINTER, 2021-2025) - CZECHIA
# ============================================

cat("\n========================================\n")
cat("4. PROCESSING SENTINEL (WEEKLY WINTER) - CZECHIA\n")
cat("========================================\n\n")
...
if(!is.null(df_sentinel)) {
  ...
  cat("✓ Czechia sentinel:", nrow(czechia_sentinel), "rows\n")
  
  # Define Winter weeks
  czechia_sentinel <- czechia_sentinel %>%
    mutate(
      is_winter = ifelse(week >= 40 | week <= 9, TRUE, FALSE)
    ) %>%
    filter(is_winter & !is.na(week))
  
  cat("  Winter weeks:", nrow(czechia_sentinel), "\n")
  
  if(nrow(czechia_sentinel) > 0) {
    sentinel_wide <- czechia_sentinel %>%
...
# ============================================
# 5. CREATE VACCINE WEEKLY DATA (WITH INTERPOLATION) - CZECHIA
# ============================================

cat("\n========================================\n")
cat("5. CREATING VACCINE WEEKLY DATA (INTERPOLATED) - CZECHIA\n")
cat("========================================\n\n")
...
# ============================================
# 6. MERGE ALL DATA - CZECHIA
# ============================================

cat("\n========================================\n")
cat("6. MERGING ALL DATA - CZECHIA\n")
cat("========================================\n\n")

# Temperature weekly
czechia_temp_weekly <- temp_avg_all %>%
  mutate(
    year = year(date),
    week = week(date)
  ) %>%
...
full_data <- excess_winter %>%
  left_join(czechia_temp_weekly, by = c("year", "week")) %>%
  left_join(czechia_cbr %>% dplyr::select(year, cbr), by = "year") %>%
...
# ============================================
# 7. REGRESSION - EXCESS MORTALITY (WITH INTERPOLATED VACCINE) - CZECHIA
# ============================================

cat("\n========================================\n")
cat("7. REGRESSION - EXCESS MORTALITY - CZECHIA\n")
cat("========================================\n\n")
...
if(nrow(full_data) >= 50) {
  ...
  # Model Comparison
  cat("\n========================================\n")
  cat("MODEL COMPARISON - EXCESS MORTALITY - CZECHIA\n")
  cat("========================================\n\n")
  ...
  cat("\n========================================\n")
  cat("EFFECTS - EXCESS MORTALITY - CZECHIA\n")
  cat("========================================\n\n")
...
# ============================================
# 8. REGRESSION - SENTINEL VALIDATION - CZECHIA
# ============================================

cat("\n========================================\n")
cat("8. REGRESSION - SENTINEL VALIDATION - CZECHIA\n")
cat("========================================\n\n")
...
if(nrow(validation_data) >= 20) {
  ...
  if(nrow(validation_vaccine) >= 10) {
    ...
    cat("\n========================================\n")
    cat("EFFECTS - SENTINEL VALIDATION - CZECHIA\n")
    cat("========================================\n\n")
...
# ============================================
# 9. FINAL CONCLUSION - CZECHIA
# ============================================

cat("\n========================================\n")
cat("9. FINAL CONCLUSION - CZECHIA\n")
cat("========================================\n\n")

cat("ANALYSIS: Czechia\n")
...
if(exists("effects_excess") && nrow(effects_excess) > 0) {
  ...
  cat("\n========================================\n")
  cat("OVERALL CONCLUSION - CZECHIA\n")
  cat("========================================\n\n")
